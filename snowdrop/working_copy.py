...
class WorkingCopy_GPKG_0(WorkingCopyGPKG):
    ...
    def diff_db_to_tree(self, dataset):
        ...
        with self.session() as db:
            o = diff.db_to_tree(self.repo, self.table, db, dataset.tree)
            return diff.Diff(
                dataset,
                meta=o['META'],
                inserts=o['I'],
        ...
        with self.session() as db:
            ...
            if action == "META":
                ...
                dbcur.execute(
                    ...
                    (dataset.name, feature_key, obj[pk_field]),
                )
                ...
                # updated
                dbcur.execute(
                    "UPDATE __kxg_map SET state=0 WHERE table_name=? AND state != 0;", (dataset.name,)
                )
