...
class TableWorkingCopy(WorkingCopyPart):
    ...
    def get_diff_for_table_creation(self, table, meta_items):
        ...
        with self.session():
            ...
            feature_diff = self.all_features_diff(
                table, schema_value, delta_type=Delta.insert
            )
    ...
    def diff_dataset_to_working_copy_feature(
        ...
        if (
            ...
            return dataset.all_features_diff(
                delta_type=Delta.delete, flags=WORKING_COPY_EDIT
            ) + self.all_features_diff(
                dataset.table_name, schema, delta_type=Delta.delete
            )
    ...
    def write_full(self, target_commit, *datasets):
        """
    ...
    def delete_features(self, sess, repo_key_filter, track_changes_as_dirty=True):
        ...
        base_datasets = self.repo.datasets(
            base_tree_id, filter_dataset_type=self.SUPPORTED_DATASET_TYPE
        )
    ...
    def _do_reset_datasets(
        ...
        *,
        ...
        with self.session() as sess:
            ...
            if ds_inserts:
                self.write_full(
                    target_commit, *[target_datasets[d] for d in ds_inserts]
                )
    ...
    def _find_unsupported_updates(self, ds_updates, base_datasets, target_datasets):
        ...
        to be rewritten from scratch (eg, many types of schema changes are unsupported except as full rewrites).
        """
