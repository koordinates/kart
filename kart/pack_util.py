...
def write_to_packfile(repo, mark_as_promisor=None):
    ...
    pack_file_path_str = mempack_backend.dump_to_dir(str(objects_path))
    if mark_as_promisor:
        pack_file_path = objects_path / pack_file_path_str
        pack_file_path.with_name(pack_file_path.stem + ".promisor").touch()
