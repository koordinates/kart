...
fn swap_geometry(wkb: &[u8], pos: &mut usize, out: &mut Vec<u8>) -> Result<()> {
    if *pos + 5 > wkb.len() {
        return Err(Error::Format("WKB truncated at geometry header".to_string()));
    }
...
mod tests {
    ...
    const POINT_1_2: &str =
        "47500001000000000101000000000000000000f03f0000000000000040";
    ...
    const POINT_EMPTY: &str =
        "47500011000000000101000000000000000000f87f000000000000f87f";
